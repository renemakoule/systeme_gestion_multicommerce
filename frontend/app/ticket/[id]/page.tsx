  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-slate-400">
        ...
        <p className="text-sm font-medium uppercase tracking-widest animate-pulse">Chargement de votre e-ticket...</p>
      </div>
    );
  }
  ...
  if (error || !ticketData) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-slate-500 max-w-xs mx-auto mb-6">{error || "Nous n'avons pas pu trouver ce ticket."}</p>
        <button 
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-10 px-4 sm:px-6">
      <div className="w-full max-w-[72mm] bg-white shadow-2xl rounded-sm overflow-hidden transform transition-all hover:scale-[1.01]">
        ...
        <div className="bg-slate-900 text-white p-4 text-center">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-80">E-Ticket Officiel</h2>
          <p className="text-[8px] font-medium opacity-40 mt-1 uppercase tracking-wider">Certifié par GAS Systeme</p>
        </div>
      ...
      <footer className="mt-12 text-center space-y-1">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.4em]">GAS Systeme v2</p>
        <p className="text-[7px] text-slate-300 font-medium lowercase">powered by antigravity ai</p>
      </footer>
    </div>
  );
}
